...
- `MinimapZoom.lua` polls movement every 0.5 seconds and refreshes immediately on combat transitions. It uses a fixed medium zoom during combat and skips secret movement values otherwise. Target changes schedule discrete eased zoom steps over 0.45 seconds; an animation generation invalidates pending steps from previous targets.
