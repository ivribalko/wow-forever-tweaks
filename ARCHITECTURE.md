...
- `QuestMacros.lua` adapts UITweaks quest cycling to the native Forever APIs. Slash commands select the next or previous watched quest in tracker order, falling back to watch-list order. Login creates or updates two account macros; combat defers writes, and full macro storage retries after macro changes. No navigation artwork or setup button is created.
